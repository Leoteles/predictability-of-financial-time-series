...
%   URissanen = 'dl1(mss,a,deltas,dim_x)';                     --- pseudo-linear DL (unscaled params)
%   URissanen2= 'dl2(mss,a,deltas,dim_x,base,basic,offset,v)'; --- approx DL (unscaled params)
%
%
...
% the scaling factor. Similarly best_deltas are scaled by the same factor. This prevents (i.e. reduces
...
if trace>0
   fig1 = figure;
   disp(sprintf('Displaying on figure %d',fig1));
   set(fig1,'Position',[4 391 1020 310]);
   subplot(211);
...
while k<ny
   ...
   while j <= k 										% to expand model
      ...
      %save([filename,'-j',int2str(j),'-k',int2str(k)]);   
      save([filename,'-k',int2str(k)]);   
      ...
      if i>offset,             						% is new entrant nonlinear?
         ...
         if wobble,       
            ...
            newnewbase=newbase;
            [newbase,thisphi,thisscale,improved]=rb_wobble(newbase,X,v,func,e,meth);
