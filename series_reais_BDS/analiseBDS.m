function [estrutura, prev] = analiseBDS(serie,Nsur)

%Parametros para a funcao BDS
maxdim = 2;
distance = 1.5;
flag = 0;
maxram = 500;%Importante ajustar maxram para a quantidade disponivel de memoria, senao, trava


z = geraSurrogates(serie,'alg0',Nsur);

%Calcula BDS para os Surrogates
for n = 1:Nsur
    %Mostra status atual do calculo
    if (~mod(n,10) || n==Nsur)
        str = sprintf('Calculando BDS para sub-rogados... %g%% concluido', (100*n)/Nsur);
        disp(str);
    end
    
    y = z(:,n);
    %Calcula a estatística BDS para a série n
    [W, SIG, C, C1, K] = bds (y, maxdim, distance, flag, maxram);
    w_sur(n) = W;
end

%Calcula a estatística BDS para a série original
disp('Calculando BDS para série original.');
y = serie;
[W, SIG, C, C1, K] = bds (y, maxdim, distance, flag, maxram);
w_serie = W;

if Nsur > 0
    %Compara bds da serie e surrogates
    Nbins = 20;
    [n,x] = hist(w_sur(:),Nbins);
    ...
    hold on;
    line([w_serie w_serie],[0 max(n)*1.3],'Color','r','LineWidth',3);%Linha que indica bds para a série original
    str = sprintf('BDS Série Orignal: %g', w_serie);
    h_legend = legend('Histograma W_2 (permutações)',str,'location','northeast');
    set(h_legend,'FontSize',14);
    xlabel('W_2','fontsize',14);
    ylabel('Distribuição de W_2 (permutações)','fontsize',14);
    hold off;
    
    media_sur = mean(w_sur);
    std_sur = std(w_sur);

    prev = abs((w_serie - media_sur)) ./ std_sur;

    estrutura.w_sur = w_sur;

else
    prev = NaN;
end

estrutura.w_serie = w_serie;
...
% REFERENCES:
%
% Brock, William, Davis Dechert & JosÈ Scheinkman (1987), "A Test for Independence
%    Based on the Correlation Dimension", University of Wisconsin-Madison, Social
%    Science Research Working Paper, no. 8762
%
% Brock, William, Davis Dechert, JosÈ Scheinkman & Blake LeBaron (1996), "A test for
