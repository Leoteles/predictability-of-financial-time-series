%function [x, f] = myDE(modelo,funcao, opcao_grafica,limPop,maxGer)
%
% Onde:
...
function [x, f] = myDE(model,funcao, opcao_grafica, limP,maxGer)

global nVar nPop limPop Cr F K pF fobj modelo;
limPop = limP;
fobj = funcao;
modelo = model;
...
global limPop nVar nPop Cr F K pF fobj modelo;
...
while (ger<=maxGer)    
    ...
    if ger==1
        for ind = 1:nPop
            modelo.sigma = populacao(:,ind,ger);
            f(1,ind) = feval(fobj,modelo);
        end
    end
