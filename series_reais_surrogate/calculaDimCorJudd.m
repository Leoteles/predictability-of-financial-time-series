function [m,dcm,eps0,cim]=calculaDimCorJudd(y,de,tau,nbins,nt,dt)

% function [m,dc,eps0,ci] = calculaDimCorJudd(y,de,tau,nbins,nt,dt);
...
pretty=0;  %pictures?
%%%%%%%%%%%%%ALTERADO
noccup=2;%20 % minimum number of occupied bins to fit to.
%%%%%%%%%%%%%ALTERADO
...
for mi=1:nde,
    ...
    while(sum(diff(ci(ind))>0)>noccup), %keep going so long as noccup
      ...
      %judd_da retorna o erro rms ou se ajustar epsilon a ci usando 'd' e
      %'a'
      %xi=fminsearch('judd_da',xi,opt,bins(ind),ci(ind)); % do it once (to
      %est. d)
      fhandle = @judd_da;%Linhas abaixo alteradas para que se execute todas as func. como subfunc.
      xi=fminsearch(fhandle,xi,opt,bins(ind),ci(ind)); % do it once (to est. d)
      xi=[xi(1) xi(2) zeros(1,dt-1)];
      %[xi,gfit,exitflag]=fminsearch('judd_da',xi,opt,bins(ind),ci(ind)); % do it again
      [xi,gfit,exitflag]=fminsearch(fhandle,xi,opt,bins(ind),ci(ind)); % do it again
